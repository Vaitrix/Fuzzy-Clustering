...
% below).
% 11_Interphase_extract_small_channel_1pc_PSB_0p3uM_Taxol_01h43/Asp1.tif

im=imread('11_Interphase_extract_small_channel_1pc_PSB_0p3uM_Taxol_01h43/Asp1p5.tif', 35); % sample image
%im = ref_image;
[C,U,LUT,H]=FastFCMeans(im,2); % perform segmentation V- also changed 3 to 5
...
c={'-r' '-g' '-b'};
for i=1:2 %V- optional change 3 to 5/2.
...
L=LUT2label(im,LUT);
Lrgb=zeros([numel(L) 3],'uint8'); % V- 3 to 5
for i=1:2 % changed 3 to 5
    Lrgb(L(:)==i,i)=255;
end
Lrgb=reshape(Lrgb,[size(im) 3]); % V- 3 to 5

subplot(1,2,2), imshow(Lrgb,[]);
set(get(gca,'Title'),'String','FUZZY C-MEANS (C=5)'); % V- 3 to 5

% If necessary, you can also unpack the membership functions to produce 
% membership maps
Umap=FM2map(im,U,H);
figure('color','w');
for i=1:5 %V- change from 5 to 3.
    subplot(2,5,i), imshow(Umap(:,:,i)); %V- changed from 3 to 5
